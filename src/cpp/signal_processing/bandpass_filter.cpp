...
void rbj_lowpass(double f0, double fs, double Q, double& b0, double& b1, double& b2, double& a0,
                 double& a1, double& a2) {
    ...
    const double a1l = -2.0 * cosw0;
    const double a2l = 1.0 - alpha;
    b0 = b0l / a0l;
    b1 = b1l / a0l;
    b2 = b2l / a0l;
    a0 = 1.0;
...
void rbj_highpass(double f0, double fs, double Q, double& b0, double& b1, double& b2, double& a0,
                  double& a1, double& a2) {
    const double w0 = 2.0 * kPi * f0 / fs;
    const double cosw0 = std::cos(w0);
    const double sinw0 = std::sin(w0);
    ...
    const double a1h = -2.0 * cosw0;
    const double a2h = 1.0 - alpha;
    b0 = b0h / a0h;
    b1 = b1h / a0h;
    b2 = b2h / a0h;
    a0 = 1.0;
...
BandpassChain::BandpassChain(double fs_hz, double fl_hz, double fh_hz) {
    ...
    rbj_highpass(fl_hz, fs_hz, 0.707, b0, b1, b2, a0, a1, a2);
    stages_.emplace_back(b0, b1, b2, a0, a1, a2);
    rbj_lowpass(fh_hz, fs_hz, 0.707, b0, b1, b2, a0, a1, a2);
    stages_.emplace_back(b0, b1, b2, a0, a1, a2);
}

void BandpassChain::reset() {
    stages_.clear();
    const double fs = 200.0;  // reset reconstruct - need store fs/fl/fh
}
